	...
	do{
		...
		for (int i = 0; i < m.size(); i++) {
			if ((startTime + period) <= currentTime) {
				//cout << "VOLTA COMPLETA: startTime: " << startTime << ", currentTime: " << currentTime << endl;
				startTime = currentTime;
			}

			if (i == 1) {
				float angle = (360.f * (currentTime - startTime)) / period;

				//cout << angle << endl;

				glm::mat4 aux = glm::mat4(1.0);

				aux = glm::rotate(aux, angle, glm::vec3(0, 1, 0));

				//currentAngle = angle;

				aux = glm::translate(aux, glm::vec3(4, 0, 0));

				aux = glm::rotate(aux, 90.f, glm::vec3(0, -1, 0));
