...
int main(void)
{
	...
	// Load the texture
	GLuint Texture = loadBMP_custom("mesh/brasil.bmp");
	...
	bool res = loadOBJ("mesh/flag.obj", vertices, uvs, normals);
	...
	GLuint angleID = glGetUniformLocation(programID, "angle");
	...
	do{
		...
		if ((startTime + periodo) > currentTime) {
			float angle = (360 * (currentTime - startTime)) / periodo;
