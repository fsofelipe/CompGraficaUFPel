...
MAT.: 12101864
ALL FUNCIONS USED IN THIS PROJECT

references :
http://www.cs.princeton.edu/courses/archive/fall08/cos526/lec/526-08-simplify.pdf
...
int main(void) {
	...
	Mesh objeto = Mesh("mesh/suzanne.obj");
	
	std::stack <State> myStack;
	myStack.push(State(objeto.getIndices(), objeto.getVertices(), objeto.getPriorList()));
	cout << "Inseriu: " << myStack.size() << endl;
	//objeto.printPriorList();
	...
	glBufferData(GL_ARRAY_BUFFER, objeto.getVertices().size() * sizeof(glm::vec3), &objeto.getVertices()[0], GL_STATIC_DRAW);

	GLuint uvbuffer;
	glGenBuffers(1, &uvbuffer);
	glBindBuffer(GL_ARRAY_BUFFER, uvbuffer);
	glBufferData(GL_ARRAY_BUFFER, objeto.getUvs().size() * sizeof(glm::vec2), &objeto.getUvs()[0], GL_STATIC_DRAW);

	GLuint normalbuffer;
	glGenBuffers(1, &normalbuffer);
	glBindBuffer(GL_ARRAY_BUFFER, normalbuffer);
	glBufferData(GL_ARRAY_BUFFER, objeto.getNormals().size() * sizeof(glm::vec3), &objeto.getNormals()[0], GL_STATIC_DRAW);

	// Generate a buffer for the indices as well
	GLuint elementbuffer;
	glGenBuffers(1, &elementbuffer);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, elementbuffer);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, objeto.getIndices().size() * sizeof(unsigned short), &objeto.getIndices()[0], GL_STATIC_DRAW);
	...
	do {
		...
		if (glfwGetKey(g_pWindow, GLFW_KEY_COMMA) == GLFW_PRESS) {
			if (glfwGetKey(g_pWindow, GLFW_KEY_COMMA) == GLFW_RELEASE) {
		...
		if (glfwGetKey(g_pWindow, GLFW_KEY_PERIOD) == GLFW_PRESS) {
			if (glfwGetKey(g_pWindow, GLFW_KEY_PERIOD) == GLFW_RELEASE) {
				if (myStack.size() > 0) {
					State temp = myStack.top();
					myStack.pop();

					objeto.setIndex(temp.getIndexes());
					objeto.setIndexedVertex(temp.getIndexedVertex());
					objeto.setPriorList(temp.getPriorList());

					cout << "Retirou: " << myStack.size() << endl;
				}
			}
		...
		/* CHAMADA PARA REDUZIR A MESH AQUI */
		/* DEVE ALTERAR: */
